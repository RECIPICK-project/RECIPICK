 ...
 * LIKE_TABLE 매핑 엔티티
 *
 * <p>✅ 설계 의도 - PK: like_id (AUTO_INCREMENT) - FK1: user_id → USER.user_id (다대일) - FK2: post_id →
 * POST.post_id (다대일) - createdAt: 좋아요 누른 시점 기록
 *
 * <p>⚠️ 주의 - 테이블 이름은 ERD 기준으로 LIKE_TABLE (대문자) - 한 유저가 한 게시글에 여러 번 좋아요 못 누르도록 하려면 UNIQUE 제약 필요
 * (user_id + post_id)
 */
@Entity
@Getter
