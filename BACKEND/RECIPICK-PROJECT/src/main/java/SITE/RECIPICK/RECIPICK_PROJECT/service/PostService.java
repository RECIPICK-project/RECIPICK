...
public class PostService {
    ...
    public PostDto saveRecipe(PostDto postDto) {
        ...
        try {
            // 첫 게시글인지 확인 - 기존 메서드 활용
            long userPostCount = postRepository.countPublishedByAuthor(userInfo.userId);

            if (userPostCount == 1) {
                // 첫 게시글이면 보너스 포인트
                pointService.addPoints(userInfo.userId, PointService.PointAction.FIRST_POST);
            } else {
                // 일반 게시글 포인트
                pointService.addPoints(userInfo.userId, PointService.PointAction.POST_CREATE);
            }

            log.info("게시글 작성 포인트 적립 완료 - userId: {}, postId: {}",
                    userInfo.userId, savedEntity.getPostId());
        } catch (Exception e) {
            log.warn("포인트 적립 실패 (게시글 작성은 성공) - userId: {}, error: {}",
                    userInfo.userId, e.getMessage());
        }
