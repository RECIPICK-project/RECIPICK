...
function renderMain(){
  ...
  minus.onclick = () => {
    ...
    if (idx > -1) { selected[idx].main = false; render(); }
  };
}
...
function renderList(){
  ...
  selected.forEach((it, idx) => {
    ...
    row.innerHTML = `
      <div class="check" role="button" tabindex="0" aria-pressed="${it.main ? 'true':'false'}" title="${it.main ? '메인' : '서브'}"></div>
    ...
    chk.addEventListener('keydown', (e)=>{ if(e.key==='Enter'||e.key===' ') { e.preventDefault(); setMain(); } });
...
picker.addEventListener('click', (e)=>{
  if (e.target.hasAttribute('data-close')) picker.setAttribute('aria-hidden','true');
});
...
searchBtn.addEventListener('click', () => {
  ...
  const main = selected.find(x=>x.main)?.name || '';
  const subs = selected.filter(x=>!x.main).map(x=>x.name);
  // TODO: 실제 검색 페이지로 라우팅
  alert(`검색 실행\n메인: ${main}\n서브: ${subs.join(', ')}`);
});
