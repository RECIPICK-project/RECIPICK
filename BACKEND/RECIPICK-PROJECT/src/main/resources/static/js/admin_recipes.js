...
(() => {
  ...
  function getCookie(name) {
    return document.cookie.split('; ').map(v => v.trim()).find(
        r => r.startsWith(name + '='))?.split('=')[1];
  }
  ...
  document.getElementById('postList')?.addEventListener('click', (e) => {
    ...
    location.assign(btn.dataset.url);
  }, true);
  ...
  function qualifiesByRule(r, m) {
    ...
    const fails = [];
    if (+m.days > +r.windowDays) {
      fails.push('days');
    }
    if (+m.likes < +r.minLikes) {
      fails.push('likes');
    }
    if (+m.saves < +r.minSaves) {
      fails.push('saves');
    }
    if (+m.rating < +r.minRating) {
      fails.push('rating');
    }
    if (+m.reviews < +r.minReviews) {
      fails.push('reviews');
    }
    if (+m.views < +r.minViews) {
      fails.push('views');
    }
  ...
  function bindBulkActions() {
    ...
    $('#delPosts')?.addEventListener('click', async () => {
      ...
      if (!confirm(`삭제하시겠습니까? (${ids.length}개)`)) {
        return;
      }
      try {
        for (const id of ids) {
  ...
  function openPromoteModal(ids, m) {
    ...
    if (m) {
      ...
      if (rule) {
        ...
        lines.push(`<div style="margin-top:6px; font-size:13px; color:${ok
            ? 'var(--green-700)' : '#E53935'}">
  ...
  async function loadPosts() {
    const res = await jfetch(postsAPI(state.post),
  ...
  function renderPostPager(hasMore) {
    ...
    pager.innerHTML = `
      ...
      <span class="page-info">${Math.floor(state.post.offset / state.post.limit)
    + 1}</span>
  ...
  function renderCommentPager(hasMore) {
    ...
    pager.innerHTML = `
      ...
      <span class="page-info">${Math.floor(
        state.comment.offset / state.comment.limit) + 1}</span>
      <button class="mini" ${hasMore ? '' : 'disabled'} id="cNext">다음</button>
    `;
    $('#cPrev')?.addEventListener('click', () => {
      if (state.comment.offset > 0) {
