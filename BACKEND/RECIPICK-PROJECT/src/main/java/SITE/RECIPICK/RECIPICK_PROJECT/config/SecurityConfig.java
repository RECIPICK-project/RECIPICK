...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.cors(cors -> cors.configurationSource(corsConfigurationSource()))
                ...
                .authorizeHttpRequests(
                        auth ->
                                auth
                                        ...
                                        //            // 정적 리소스 및 페이지 허용 (pages 폴더 경로 반영)
                                        //            .requestMatchers("/", "/pages/home.html",
                                        // "/pages/login.html",
                                        //                "/pages/signup.html",
                                        ...
                                        // 이메일 인증 API
                                        .requestMatchers("/api/auth/email/**")
                                        .permitAll()
                                        ...
                                        .requestMatchers("/pages/main.html")
                                        .hasAnyRole("USER", "ADMIN")
                                        .requestMatchers("/pages/admin.html")
                                        .hasRole("ADMIN")

                                        // 관리용 API
                                        .requestMatchers("/api/users/all", "/api/users/set-active")
                                        .hasRole("ADMIN")
                ...
                .formLogin(
                        form ->
                                form.loginPage("/pages/login.html") // pages 폴더 경로로 수정
                                        ...
                                        .failureHandler(
                                                (req, res, ex) -> {
                                                    ...
                                                    res.sendRedirect(
                                                            "/pages/login.html?error="
                                                                    + msg); // pages 폴더 경로로 수정
                                                })
                                        .permitAll())
                .oauth2Login(
                        oauth ->
                                oauth.loginPage("/pages/login.html") // pages 폴더 경로로 수정
                                        .userInfoEndpoint(
                                                u ->
                                                        u.userAuthoritiesMapper(
                                                                authorities -> {
                                                                    java.util.Set<
                                                                                    org
                                                                                            .springframework
