...
public class ReviewService {
    ...
    public ReviewDto createReview(Integer userId, ReviewDto reviewDto) {
        ...
        try {
            // 첫 리뷰인지 확인
            long userReviewCount = reviewRepository.countByUserUserId(userId);

            if (userReviewCount == 1) {
                // 첫 리뷰면 보너스 포인트
                pointService.addPoints(userId, PointService.PointAction.FIRST_REVIEW);
            } else {
                // 일반 리뷰 포인트
                pointService.addPoints(userId, PointService.PointAction.REVIEW_CREATE);
            }

            log.info("리뷰 작성 포인트 적립 완료 - userId: {}, reviewId: {}",
                    userId, savedReview.getReviewId());
        } catch (Exception e) {
            log.warn("포인트 적립 실패 (리뷰 작성은 성공) - userId: {}, error: {}",
                    userId, e.getMessage());
        }
