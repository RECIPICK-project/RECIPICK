 ...
 *
 * <p>컨트롤러(MyPostController)에서 호출되며, - offset/limit → PageRequest 변환 - Repository에서 조건(정식/임시, 최신순)으로
 * 조회 - Post 엔티티 → PostDTO 매핑 의 과정을 수행한다.
 */
@Service
...
public class MyPostService {
     ...
     *     <p>1) PageRequest.of(page, size) 생성 - page = offset / max(1, limit) - size = max(1,
     *     limit) (0 들어와도 방어) 2) postRepo.findByUserIdAndRcpIsOfficialTrueOrderByCreatedAtDesc(...)
     *     - 내 글 중 rcp_is_official = true 인 것만 - created_at 내림차순 3) 엔티티 → DTO 매핑 후 반환
     */
    ...
    public List<PostDto> getMyOfficialPosts(Integer me, int offset, int limit) {
        ...
        return postRepo.findByUserIdAndRcpIsOfficialOrderByCreatedAtDesc(me, 1, page).stream()
     ...
     *     <p>구현 포인트는 getMyOfficialPosts와 동일하되, rcp_is_official = false 조건을 사용한다.
     */
    @Transactional(readOnly = true)
    public List<PostDto> getMyTempPosts(Integer me, int offset, int limit) {
        var page = PageRequest.of(offset / Math.max(limit, 1), Math.max(limit, 1));
        return postRepo.findByUserIdAndRcpIsOfficialOrderByCreatedAtDesc(me, 0, page).stream()
                .map(PostMapper::toDto)
                .toList();
    }
}
