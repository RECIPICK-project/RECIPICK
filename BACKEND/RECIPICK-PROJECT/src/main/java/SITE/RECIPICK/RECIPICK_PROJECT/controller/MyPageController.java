 ...
 * 마이페이지 컨트롤러
 *
 * <p>역할 - 현재 로그인 사용자의 마이페이지 관련 API 집합
 *
 * <p>인증 - 사용자 식별자는 SecurityContext에서 가져온다(AuthUtil)
 *
 * <p>예외 처리 - 현재는 런타임 예외가 그대로 전달될 수 있음. 운영 전 @ControllerAdvice 로 400/403/404/409 등의 에러 포맷 표준화 권장.
 */
@RestController
...
public class MyPageController {
     ...
     * 프로필 조회
     *
     * <p>반환 필드 - 닉네임, 등급, 프로필 이미지 - 내가 올린 정식 레시피 개수 - 내 정식 레시피의 총 좋아요 수 - 활동 수(리뷰 + 댓글)
     */
    @GetMapping("/profile")
    @Operation(
            summary = "내 프로필 조회",
     ...
     * 닉네임 변경(7일 제한)
     *
     * <p>규칙 - 공백/길이(≤50) 검증 - 기존 닉네임과 동일이면 변경 없음 - updated_at 기준 7일 이내면 거부
     * (NICKNAME_CHANGE_COOLDOWN) - 닉네임 중복이면 거부 (NICKNAME_DUPLICATED)
     *
     * <p>성공 시 204(No Content) 반환
     */
    ...
    @ApiResponses({
        ...
        @ApiResponse(
                responseCode = "400",
                description = "요청 값 오류(NICKNAME_REQUIRED/NICKNAME_TOO_LONG 등)"),
        @ApiResponse(
                responseCode = "409",
                description = "쿨다운 또는 중복(NICKNAME_CHANGE_COOLDOWN / NICKNAME_DUPLICATED)"),
        @ApiResponse(responseCode = "404", description = "프로필 없음(PROFILE_NOT_FOUND)"),
        @ApiResponse(responseCode = "500", description = "서버 오류")
    })
    public ResponseEntity<Void> changeNickname(
            @Parameter(description = "변경할 닉네임", required = true) @RequestBody
                    NicknameUpdateRequest req) {
