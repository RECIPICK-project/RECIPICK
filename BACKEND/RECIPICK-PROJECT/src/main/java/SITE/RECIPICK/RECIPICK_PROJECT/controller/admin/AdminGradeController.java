 ...
 *
 * <p>✅ 목적 - 관리자 전용 "유저 등급(Grade) 관리" API 전담 컨트롤러. - URL prefix: /admin/users
 *
 * <p>✅ 보안 - 모든 엔드포인트는 ROLE_ADMIN 권한 필요(@PreAuthorize). - SecurityConfig 에서 /admin/** 요청을
 * hasRole('ADMIN') 으로 제한해야 함.
 *
 * <p>✅ 연동 - AdminGradeService.updateUserGrade(userId, req) 에 도메인 검증/변경 로직 위임.
 *
 * <p>✅ 요청/응답 규격 - PATCH /admin/users/{userId}/grade - Path: userId (Integer) : 등급을 변경할 대상 사용자 ID -
 ...
 *
 * <p>✅ 예외 처리(권장) - @ControllerAdvice(GlobalExceptionHandler)에서 IllegalArgumentException → 400
...
public class AdminGradeController {
     ...
     * 유저 등급 변경
     *
     * <p>예) PATCH /admin/users/12/grade Body: { "grade": "SILVER" }
     *
     * <p>보안: ROLE_ADMIN 필수
     */
    ...
    public ResponseEntity<Void> changeGrade(
            @PathVariable Integer userId, @RequestBody GradeUpdateRequest req) {
