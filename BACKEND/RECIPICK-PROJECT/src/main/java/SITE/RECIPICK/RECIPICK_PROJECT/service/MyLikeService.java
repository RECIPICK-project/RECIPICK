 ...
 *
 * <p>컨트롤러(MyLikeController)에서 호출되며, Repository → Entity(Post) → DTO(PostDTO) 변환 과정을 담당한다.
 */
@Service
...
public class MyLikeService {
     ...
     *     <p>1) offset과 limit을 기반으로 PageRequest 생성 - PageRequest.of(page, size) 구조인데, page = offset
     *     / limit 으로 환산 - Math.max(1, limit) → limit이 0 들어와도 최소 1개는 조회하도록 방어
     *     <p>2) postRepo.findLikedPosts(me, pageable) 호출 - 내부적으로 JOIN(LikeTable + Post) 쿼리 실행 - 내가
     *     좋아요한 Post 엔티티 목록 반환
     *     <p>3) Entity(Post) → DTO(PostDTO) 변환 - PostMapper::toDto 메서드로 매핑 -
     *     stream().map(...).collect(toList()) 형태
     *     <p>4) 최종적으로 JSON 응답에 쓰일 DTO 리스트 리턴
     *     <p>트랜잭션 속성: @Transactional(readOnly = true) → SELECT 전용, 성능 최적화
     */
    ...
    public List<PostDto> getMyLikedPosts(Integer me, int offset, int limit) {
        ...
        return postRepo.findLikedPosts(me, pageable).stream().map(PostMapper::toDto).toList();
    }
}
